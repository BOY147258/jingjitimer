...
export class Sync {
  ...
  async join(room, role, serverHost) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Use custom server host if provided (e.g. Cloudflare Tunnel URL), otherwise current origin
      const targetHost = serverHost
        ? serverHost.replace(/^https?:\/\//, '').split('/')[0]
        : location.host;
      const url = `${proto}//${targetHost}/ws?room=${encodeURIComponent(room)}&role=${encodeURIComponent(role)}`;
